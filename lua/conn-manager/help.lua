...
local function compute(map)
  local head_lhs = 'conn-manager mappings'
  local head_rhs1 = 'exit: q'
  local head_rhs2 =
    string.format('sort by %s: s', config.help.sort_by == 'key' and 'description' or 'keymap')

  -- formatted lhs and desc from active keymap
  local mappings = vim.tbl_map(
    function(m) return { lhs = tidy_lhs(m.lhs), desc = tidy_desc(m.desc) } end,
    map
  )
  ...
  if config.help.sort_by == 'desc' then
    sort_fn = function(a, b) return a.desc:lower() < b.desc:lower() end
  else
    -- by default sort roughly by lhs
    sort_fn = function(a, b) return sort_lhs(a.lhs, b.lhs) end
  end
...
local function open()
  ...
  M.winnr = vim.api.nvim_open_win(
    M.bufnr,
    true,
    vim.tbl_deep_extend('force', config.help.window_config, {
      width = width,
      height = #lines,
    })
  )
  ...
  local function toggle_sort()
    config.help.sort_by = (config.help.sort_by == 'desc') and 'key' or 'desc'
    open()
  end

  -- hardcoded
  local help_keymaps = {
    q = { fn = close, desc = 'exit help' },
    ['<Esc>'] = { fn = close, desc = 'exit help' }, -- hidden
    s = { fn = toggle_sort, desc = 'toggle sorting method' },
  }

  -- api help binding closes
  for _, m in ipairs(map) do
    if m.callback == M.toggle then
      help_keymaps[m.lhs] = { fn = close, desc = 'conn-manager: exit help' }
    end
  end
